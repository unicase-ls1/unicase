...
public final class WorkItemTransferOperator {
	...
	public static void moveWorkItems(List<WorkItem> selectedWorkItems,
		...
		try {
			progressDialog.open();
			progressDialog.getProgressMonitor().beginTask("Move workitems...",
					100);
			...
			for (WorkItem selectedWorkItem : selectedWorkItems) {
				...
				// all other WorkItems move
					...
					targetWorkPackage.getContainedWorkItems().add(
							selectedWorkItem);
				}
			...
			try {
				operationHandle.end("Move WorkItems",
						"Move " + selectedWorkItems.size()
								+ " WorkItems to WorkPackage "
								+ targetWorkPackage.getName(), ModelUtil
								.getProject(targetWorkPackage)
								.getModelElementId(targetWorkPackage));
