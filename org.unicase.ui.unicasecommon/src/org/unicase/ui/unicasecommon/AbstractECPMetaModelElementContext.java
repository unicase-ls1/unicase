 ...
 * @author helming
 */
public abstract class AbstractECPMetaModelElementContext implements
		ECPMetaModelElementContext {
		...
		if (guessedPackages == null) {
			...
			for (Entry<String, Object> entry : entries) {
				...
				if (!isKnownPackage(entry.getKey())) {
					// This is used to discover known packages which can be
					// added to
					// know packages then.
					...
					try {
						EPackage ePackage = EPackage.Registry.INSTANCE
								.getEPackage(entry.getKey());
						guessedPackages
								.addAll(getAllModelElementEClasses(ePackage));
						// BEGIN SUPRESS CATCH EXCEPTION
