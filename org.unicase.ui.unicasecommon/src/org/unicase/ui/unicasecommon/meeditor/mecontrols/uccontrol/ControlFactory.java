...
/**
 * Factory for generating {@link AbstractMEControl}'s according to a
...
public class ControlFactory {
	...
	private void initializeMEControls() {
		IConfigurationElement[] attributecontrols = Platform
				.getExtensionRegistry().getConfigurationElementsFor(
						"org.eclipse.emf.ecp.editor.attributecontrols");
		IConfigurationElement[] referencecontrols = Platform
				.getExtensionRegistry().getConfigurationElementsFor(
		...
		for (IConfigurationElement e : allControls) {
			...
			AbstractMEControl control = null;
			Class<?> resolvedType = null;
			try {
				control = (AbstractMEControl) e
						.createExecutableExtension("class");
				resolvedType = control.getClass().getClassLoader()
						.loadClass(type);
			...
			boolean showLabel = Boolean.parseBoolean(e
					.getAttribute("showLabel"));
			control.setShowLabel(showLabel);
			ArrayList<AbstractMEControl> list = controlRegistry
					.get(resolvedType);
			if (list == null) {
	...
	/**
	 * Creates a {@link AbstractMEControl} according to the
	 * {@link IItemPropertyDescriptor}.
	 * 
	 ...
	 */
	public AbstractMEControl createControl(
	...
	public AbstractMEControl createControl(
			...
			EObject modelElement, ESLocalProject context) {
		...
		if (feature instanceof EAttribute) {
			...
			return createReferenceControl(itemPropertyDescriptor,
					(EReference) feature, modelElement, context);
		}

		return null;
	}

	private AbstractMEControl createReferenceControl(
			IItemPropertyDescriptor itemPropertyDescriptor, EReference feature,
			EObject modelElement, ESLocalProject context) {
		...
		AbstractMEControl control = getBestCandidate(candidates,
				itemPropertyDescriptor, feature, modelElement, context);
	...
	private AbstractMEControl createAttribute(
			IItemPropertyDescriptor itemPropertyDescriptor,
			EStructuralFeature feature, EObject modelElement,
			ESLocalProject context) {
		Class<?> instanceClass = ((EAttribute) feature).getEAttributeType()
				.getInstanceClass();
		...
		for (Class<?> clazz : keySet) {
			if (instanceClass.isPrimitive()) {
				try {
					Class<?> primitive = (Class<?>) clazz.getField("TYPE").get(
							null);
	...
	private AbstractMEControl getBestCandidate(
			ArrayList<AbstractMEControl> candidates,
			IItemPropertyDescriptor itemPropertyDescriptor,
			EStructuralFeature feature, EObject modelElement,
			ESLocalProject context) {
		int bestValue = 0;
		...
		for (AbstractMEControl abstractMEControl : candidates) {
			...
			int newValue = abstractMEControl.canRender(itemPropertyDescriptor,
					modelElement);
