...
/**
 * Abstract Dialog as pattern for all dialogs which show ModelElements for
 * selection. A
 * 
 * @author mkagel
 */
public abstract class ModelElementSelectionDialog extends
	...
	private Collection<? extends EClass> modelElements;
	private ComposedAdapterFactory adapterFactory;

	/**
	 * Constructor which calls another constructor with parameter false for
	...
	/**
	 * Contructor which calls another constructor with project parameter and
	 * false for multiple selection of elements.
	 * 
	 * @param context
	 *            the context
	 */
	public ModelElementSelectionDialog(ESLocalProject context) {
		this(context, false);
	}

	/**
	 * Constructor which calls another constructor with null for project
	 * paramter (modelelemnts will be set from outside) and the given
	 * multiSelection behaviour.
	 * 
	 ...
	 * 
	 * @param context
	 *            The context from where the model elements come from, is null
	 *            if no context is set an the model elements come from outside
	 *            the dialog
	 * @param multiSelection
	 *            indicates whether dialog allows to select more than one item
	 */
	public ModelElementSelectionDialog(ESLocalProject context,
		...
		if (context != null) {
			modelElements = (Collection<? extends EClass>) context
					.getAllModelElements();
		}
	...
	/**
	 * Constructor which calls another constructor and sets the model elements
	 * collection of the dialog only to a specified class type.
	 * 
	 * @param context
	 *            from where the model elements come from
	 * @param classType
	 ...
	 */
	public ModelElementSelectionDialog(ESLocalProject context,
			EClass classType, boolean multiSelection) {
		this(multiSelection);
		modelElements = context.getAllModelElementsByClass(classType.eClass()
				.getClass(), true);
	}
	 ...
	 */
	public ModelElementSelectionDialog(ESLocalProject context, EClass classType) {
	...
	/**
	 * Returns the ModelElement-Collection which contains the modelelements
	 * shown in the dialog.
	 * 
	 * @return Collection of modelelements for the dialog
	 */
	public Collection<? extends EClass> getModelElements() {
		return modelElements;
	}

	/**
	 * Set the ModelElement-Collection from outside. Necessary if the collection
	 * is not set by the dialog but coms from the caller.
	 * 
	 * @param modelElements
	 *            Collection which modelelements which will be shown in the
	 *            dialog
	 */
	public void setModelElements(Collection<? extends EClass> modelElements) {
