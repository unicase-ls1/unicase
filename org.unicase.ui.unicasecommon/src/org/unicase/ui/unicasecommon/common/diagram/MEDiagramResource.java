...
public class MEDiagramResource extends ResourceImpl implements Resource, Resource.Factory, Resource.Internal,
	...
	private void createNewGMFDiagram() {
		 ...
		 * (meDiagram.getType().equals(DiagramType.COMPONENT_DIAGRAM)) { id = "Component"; } else if
		 * (meDiagram.getType().equals(DiagramType.STATE_DIAGRAM)) { id = "State"; } else if
		 * (meDiagram.getType().equals(DiagramType.ACTIVITY_DIAGRAM)) { id = "Activity"; } else if
		 * (meDiagram.getType().equals(DiagramType.WORKITEM_DIAGRAM)) { id = "WorkItem"; }
		 */
