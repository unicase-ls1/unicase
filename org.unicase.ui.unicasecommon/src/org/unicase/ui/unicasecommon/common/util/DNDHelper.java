...
public final class DNDHelper {
	 ...
	 * 
	 * @param elements
	 *            all elements to drop
	 * @param diagram
	 *            the diagram
	 * @param allowedElements
	 *            elements that are allowed to drop (empty list thats going to
	 *            be filled)
	 * @return {@code true} if drop is allowed
	 */
	public static boolean canDrop(List<EObject> elements, MEDiagram diagram,
			List<EObject> allowedElements) {
		if (!elements.isEmpty()) {
			for (EObject me : elements) {
				// do not add elements that are already added and check if they
				// are allowed for the diagram
				if (!diagram.getElements().contains(me)
						&& DNDHelper.isAllowedType(diagram, me)) {
	...
	private static boolean isAllowedType(MEDiagram diagram, EObject dropee) {
		...
		for (IClientContext clientContext : clientContexts) {
			...
			// checks all types in a given context if they contain the diagram
			// and the dropped element
	...
	/**
	 * If not all elements could be dropped the user is asked whether to drop a
	 * part of them.
	 * 
	 * @param elements
	 *            all elements to drop
	 * @param allowedElements
	 *            allowed elements to drop
	 * @return {@code true} if the should be performed
	 */
	public static boolean dropMessageCheck(List<EObject> elements,
			List<EObject> allowedElements) {
