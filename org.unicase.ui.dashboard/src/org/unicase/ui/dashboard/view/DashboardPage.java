...
public class DashboardPage extends FormPage {
	...
	private final class CreateNotificationsCommand extends EMFStoreCommand {
		...
		protected void doRun() {
			String sizeProperty = getProjectSpace().getPropertyManager()
					.getLocalStringProperty(
							DashboardPropertyKeys.DASHBOARD_SIZE);
			int count = sizeProperty != null ? Integer.parseInt(sizeProperty)
					: 10;
			...
			for (int i = 0; i < count; i++) {
				...
				if (!n.isSeen()) {
					...
					if (n.getProvider().equals(UpdateNotificationProvider.NAME)) {
						entry = new UpdateDashboardEntry(DashboardPage.this,
								notificationsComposite, SWT.NONE, n,
								projectSpace);
					} else {
	...
	protected boolean sanityCheck(DashboardNotification n) {
		...
		if (n.getRelatedModelElements().size() > 0) {
			...
			EObject firstModelElement = projectSpace.getProject()
	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		IConfigurationElement[] config = Platform
				...
				.getConfigurationElementsFor("org.unicase.ui.dashboard.widgets");
		...
		Collections.sort(widgets, new Comparator<AbstractDashboardWidget>() {
			public int compare(AbstractDashboardWidget o1,
					AbstractDashboardWidget o2) {
		...
		sash.addMouseTrackListener(new MouseTrackAdapter() {
			...
			public void mouseExit(MouseEvent e) {
				sash.setBackground(Display.getCurrent().getSystemColor(
						SWT.COLOR_WHITE));
			}
		});
		...
		GridLayoutFactory.fillDefaults().numColumns(1).equalWidth(false)
				.spacing(0, 0).margins(5, 5).applyTo(notificationsComposite);
		data = new FormData();
		data.top = new FormAttachment(0, 0);
		data.bottom = new FormAttachment(100, 0);
	 ...
	 */
	public org.eclipse.emf.emfstore.internal.client.model.ProjectSpace getProjectSpace() {
		return projectSpace;
	}
