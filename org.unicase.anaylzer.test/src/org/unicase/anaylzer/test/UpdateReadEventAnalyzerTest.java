...
import org.unicase.analyzer.dataanalyzer.UpdateReadEventAnalyzer;
 ...
 *
 */
public class UpdateReadEventAnalyzerTest extends AnalyzersTest {
	private File export;
		...
		for (ProjectInfo pI : super.getProjectList()) {			
			if (pI.getName().contains("DOLLI2")) {
				...
				try {
					...
					analyzers.add(new UpdateReadEventAnalyzer());
