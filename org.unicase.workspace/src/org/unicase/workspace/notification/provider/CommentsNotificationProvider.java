	...
	private void handleOperation(AbstractOperation operation, User user, Project project) {
		...
		if (operation instanceof CreateDeleteOperation) {
			...
			if (createOperation.getModelElement() instanceof Comment) {
				ModelElement localComment = project.getModelElement(createOperation.getModelElementId());
				if (localComment != null) {
					Comment comment = (Comment) localComment;
					if (comment.getCommentedElement() instanceof Comment
						&& ((Comment) comment.getCommentedElement()).getSender() != null
						&& ((Comment) comment.getCommentedElement()).getSender().equals(user)) {
						comments.put(comment, operation.getClientDate());
					}
				}
			}
		}
