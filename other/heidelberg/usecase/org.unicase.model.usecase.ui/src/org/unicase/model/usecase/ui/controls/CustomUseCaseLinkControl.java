...
/**
 * @author phil
 * @brief Renders linked use cases in the standard view
 */
public class CustomUseCaseLinkControl extends AbstractMEControl {

	private static final String ID = "org.unicase.model.usecase.ui.controls.customusecaselinkcontrol";
	private static final String NAME = "CUSTOM USE CASE LINK CONTROL";
	...
	public int canRender(IItemPropertyDescriptor itemPropertyDescriptor, ModelElement modelElement) {
		...
		if (UseCase.class.isInstance(modelElement)
			&& ((EReference) itemPropertyDescriptor.getFeature(modelElement)).getName() == "useCases") {
			return 2;
		}
		return -1;
	}
	...
	protected Control createControl(Composite parent, int style) {
		...
		section.setText("Included Use Cases");
		...
		eAdapter = new AdapterImpl() {
			...
			public void notifyChanged(Notification msg) {
				if (msg.getFeature() instanceof EReference && ((EReference) msg.getFeature()).getName() == "useCases")
					updateControl();
			}
		};
	...
	/**
	 * Updates the control if an included use case has changed
	 */
	public void updateControl() {
		if (!mainComposite.isDisposed()) {
			...
			EList<UseCase> useCases = ((UseCase) getModelElement()).getUseCases();
			for (UseCase u : useCases) {
				IHyperlinkListener listener = new MEHyperLinkAdapter(u, getModelElement(), "useCases");
				ImageHyperlink h2 = getToolkit().createImageHyperlink(infoComposite, style);
				h2.setImage(labelProvider.getImage(u));
				h2.addHyperlinkListener(listener);
				imageHyperlinks.add(h2);
				Hyperlink h = getToolkit().createHyperlink(infoComposite, shortLabelProvider.getText(u), style);
