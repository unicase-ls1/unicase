...
public class ModelTreeContentProvider extends AdapterFactoryContentProvider {
	...
	public Object[] getElements(Object inputElement) {
		...
		// modified by Robert Heinrich
		// EPackage test = org.unicase.testspec.model.impl.ModelPackageImpl.eINSTANCE;
		// array.add(test);

		// modified by Robert Heinrich
		try {
			// get the extensions from extension point
			IConfigurationElement[] extensions = Platform.getExtensionRegistry().getConfigurationElementsFor(
				"org.unicase.modelExtension");

			for (IConfigurationElement e : extensions) {

				// get the URI form the .ecore model
				String uri = e.getAttribute("modelURI");
				String name = e.getAttribute("name");

				EPackage.Registry.INSTANCE.getEPackage(uri).setName(name);
				// get the EPackage by the URI
				EPackage model = EPackage.Registry.INSTANCE.getEPackage(uri);
