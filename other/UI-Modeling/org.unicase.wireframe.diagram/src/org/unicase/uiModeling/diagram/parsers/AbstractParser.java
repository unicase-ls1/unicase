package org.unicase.uiModeling.diagram.parsers;
...
public abstract class AbstractParser implements IParser {
	...
	protected IParserEditStatus validateNewValues(Object[] values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (value instanceof InvalidValue) {
				return new ParserEditStatus(org.unicase.uiModeling.diagram.part.UiModelingDiagramEditorPlugin.ID,
			...
			if (Boolean.TYPE.equals(iClass)) {
				if (value instanceof Boolean) {
					...
					value = new InvalidValue(NLS.bind(
						org.unicase.uiModeling.diagram.part.Messages.AbstractParser_UnexpectedValueType,
						iClass.getName()));
				}
				...
				if (value instanceof Byte) {
					...
					if (s.length() == 0) {
						...
						try {
							...
							value = new InvalidValue(NLS.bind(
								org.unicase.uiModeling.diagram.part.Messages.AbstractParser_WrongStringConversion,
								iClass.getName()));
						}
					}
				} else {
				...
				if (value instanceof String) {
					...
					if (literal == null) {
						value = new InvalidValue(NLS.bind(
							org.unicase.uiModeling.diagram.part.Messages.AbstractParser_UnknownLiteral, value));
					} else {
