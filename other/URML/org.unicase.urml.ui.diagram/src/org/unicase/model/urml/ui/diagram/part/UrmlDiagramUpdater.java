...
public class UrmlDiagramUpdater {
	...
	public static List getFeature_2012IncomingLinks(View view) {
		...
		result.addAll(getIncomingFeatureModelFacetLinks_AbstractFeature_RequieredFeatures_4037(modelElement,
			crossReferences));
	 ...
	 * @generated
	 */
	private static Collection getIncomingFeatureModelFacetLinks_AbstractFeature_RequieredFeatures_4037(
		AbstractFeature target, Map crossReferences) {
		Collection result = new LinkedList();
		Collection settings = (Collection) crossReferences.get(target);
		for (Iterator it = settings.iterator(); it.hasNext();) {
			EStructuralFeature.Setting setting = (EStructuralFeature.Setting) it.next();
			if (setting.getEStructuralFeature() == FeaturePackage.eINSTANCE.getAbstractFeature_RequieredFeatures()) {
				result.add(new UrmlLinkDescriptor(setting.getEObject(), target,
					UrmlElementTypes.AbstractFeatureRequieredFeatures_4037,
					AbstractFeatureRequieredFeaturesEditPart.VISUAL_ID));
			}
		}
		return result;
	}
	 ...
	 * @generated
	 */
	private static Collection getOutgoingFeatureModelFacetLinks_AbstractFeature_RequieredFeatures_4037(
		AbstractFeature source) {
		Collection result = new LinkedList();
		for (Iterator destinations = source.getRequieredFeatures().iterator(); destinations.hasNext();) {
			AbstractFeature destination = (AbstractFeature) destinations.next();
			result.add(new UrmlLinkDescriptor(source, destination,
				UrmlElementTypes.AbstractFeatureRequieredFeatures_4037,
				AbstractFeatureRequieredFeaturesEditPart.VISUAL_ID));
