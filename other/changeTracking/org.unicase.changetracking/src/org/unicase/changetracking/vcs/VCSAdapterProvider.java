...
/**
 * This interface must be implemented by each adapter plug-in. The
 * implementation can then be used to extend the
 * org.unicase.changetracking.vcsadapters extension point provided by this
 * plug-in. This will effectively register the adapter.
 * 
 * The provider's two main jobs are to act as a factory for the correspondent
 * adapter and to decide if this adapter is able to adapt different objects.
 * 
 * For example, the providesForProject method checks if the adapter is able to
 * provider VCS adaption for a specific project in the workspace.
 * 
 * @author jfinis
 * 
 */
public interface VCSAdapterProvider {
	 ...
	 * 
	 * This means the adapter must be able to create streams like the one
	 * provided
	 * 
	 ...
	 * revision.
	 * 
	 * This means the adapter must be able to create and check out revision like
	 * the one provided.
	 * 
	...
	/**
	 * Returns whether this class provides an adapter matching the given change
	 * package.
	 * 
	 * This means the adapter must be able to create and apply the type of
	 * packages which is provided.
	 * 
