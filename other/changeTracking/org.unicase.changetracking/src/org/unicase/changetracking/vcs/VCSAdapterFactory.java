...
/**
 * Factory class which uses the adapter registry
 * to create adapters matching for different objects.
 * 
 * For example, the createFromProject method looks
 * for an adapter matching the version control system
 * under which the project is versioned.
 * 
 * If no matching adapter can found, each method throws
 * a misuse exception.
 * 
 * @author gex
 *
...
public class VCSAdapterFactory {
	...
	/**
	 * Creates an adapter matching the version control system under which 
	...
	/**
	 * Creates an adapter matching a change package.
	 * This means that the adapter is able to apply this
	 * change package.
	 * 
	...
	/**
	 * Creates an adapter matching a release.
	 * This means the repository stream belonging to the stream
	 * of the release is supported by the version control adapter.
	 * The adapter is therefore able to build the release.
	 *
	 * 
	 * @param r a release
	 * @return the matching adapter
	 * @throws MisuseException if no matching adapter was found 
	 * or if the release doesn't have a stream or repository stream assigned
	 */
