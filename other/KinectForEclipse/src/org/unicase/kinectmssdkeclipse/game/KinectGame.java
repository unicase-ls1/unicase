...
public class KinectGame {
	...
	private void startGestureRecoginition() {
		SkeletalTracking skeletalTracking = KinectConnection.getInstance()
				.startSkeletonTracking();
		if (skeletalTracking != null) {
			final GestureRecognition gestureRecognition = skeletalTracking
			...
			Runnable gestureListener = new Runnable() {
				...
				@Override
				public void run() {
					Gesture actualGesture = gestureRecognition
							.getActualRecognizedGesture();
					while (actualGesture.equals(gestureRecognition
							.getActualRecognizedGesture())) {
						KinectGame.this.display.timerExec(-1,
								KinectGame.this.timer);
					}
				}
			};
			gestureListener.run();
