...
import org.unicase.codetrace.tracer.LineHash;
...
public class AlgorithmLineContext extends Algorithm {
	...
	private class Facet implements TracerFacet {
		...
		Facet(CodeLocation c) {
			...
			prior_hash = new int[c.linesBefore.size()];
			post_hash = new int[c.linesAfter.size()];
			
			int i;
			i=0; for(LineHash h : c.linesBefore)prior_hash[i++]=h.hash;
			i=0; for(LineHash h : c.linesAfter)post_hash[i++]=h.hash;
		...
		public void addToCodeLocation(CodeLocation c) {
			...
			l = new LinkedList<LineHash>();
			for(int i: prior_hash){
