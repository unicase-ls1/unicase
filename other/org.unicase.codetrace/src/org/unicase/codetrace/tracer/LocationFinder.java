...
public final class LocationFinder {
	...
	public CodeLocation createLocation(String projectName,
		...
		CodeLocation out = new CodeLocation();
		out.pathInProject = pathInProject;
	...
	public synchronized FoundLocation find(CodeLocation c) {
		...
		TracerFile tf = TracerFile.getFileByName(EclipseWorkspaceManager.getPathOfFile(c.projectName,c.pathInProject));
		if (tf == null){
		...
		IFile f = ResourcesPlugin.getWorkspace().getRoot().getProject(c.projectName).getFile(c.pathInProject);
