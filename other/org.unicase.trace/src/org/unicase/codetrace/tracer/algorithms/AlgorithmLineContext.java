...
public class AlgorithmLineContext extends Algorithm {
	...
	private class Facet implements TracerFacet {
		...
		Facet(CodeLocation c) {
			...
			prior_hash = new int[c.getLinesBefore().size()];
			post_hash = new int[c.getLinesAfter().size()];
			
			int i;
			i=0; for(LineHash h : c.getLinesBefore())prior_hash[i++]=h.getHash();
			i=0; for(LineHash h : c.getLinesAfter())post_hash[i++]=h.getHash();
		...
		public void addToCodeLocation(CodeLocation c) {
			...
			for(int i: prior_hash){
				LineHash lh = TraceFactory.eINSTANCE.createLineHash();
				lh.setHash(i);
				l.add(lh);
			}
