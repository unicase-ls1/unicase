	...
	private static class OpenDiagramAction extends Action {
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof StoryboardNavigatorItem) {
					selectedElement = ((StoryboardNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (StoryboardEditPart.MODEL_ID.equals(StoryboardVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		public void run() {
			...
			try {
				page.openEditor(editorInput, StoryboardDiagramEditor.ID);
			} catch (PartInitException e) {
				StoryboardDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
