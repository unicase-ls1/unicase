 ...
 * @generated
 */
public class WindowItemSemanticEditPolicy extends WireframeBaseItemSemanticEditPolicy {
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (WireframeVisualIDRegistry.getVisualID(node)) {
			case WindowWindowWidgetCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (WireframeVisualIDRegistry.getVisualID(cnode)) {
