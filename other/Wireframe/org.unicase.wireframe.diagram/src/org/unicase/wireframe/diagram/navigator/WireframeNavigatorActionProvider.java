 ...
 * @generated
 */
public class WireframeNavigatorActionProvider extends CommonActionProvider {
	...
	private static class OpenDiagramAction extends Action {
		...
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof WireframeNavigatorItem) {
					selectedElement = ((WireframeNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (PanelEditPart.MODEL_ID.equals(WireframeVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		public void run() {
			...
			try {
				page.openEditor(editorInput, WireframeDiagramEditor.ID);
			} catch (PartInitException e) {
				WireframeDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
