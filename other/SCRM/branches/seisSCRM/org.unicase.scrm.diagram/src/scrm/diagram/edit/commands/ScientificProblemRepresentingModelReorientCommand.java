...
public class ScientificProblemRepresentingModelReorientCommand extends
	...
	protected boolean canReorientSource() {
		if (!(oldEnd instanceof Mathematical_GeophysicalModel && newEnd instanceof ScientificProblem)) {
			return false;
		}
		return ScrmBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistScientificProblemRepresentingModel_4063(
	...
	protected CommandResult reorientSource() throws ExecutionException {
		getOldSource().getRepresentingModel().remove(getOldTarget());
		getNewSource().getRepresentingModel().add(getOldTarget());
	...
	protected CommandResult reorientTarget() throws ExecutionException {
		getOldSource().getRepresentingModel().remove(getOldTarget());
		getOldSource().getRepresentingModel().add(getNewTarget());
		return CommandResult.newOKCommandResult(referenceOwner);
	}
