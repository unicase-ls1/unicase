 ...
 * Upon double-clicking a SCRMSpace-Node, the representing SCRMDiagram is
 * opened, containing all of the SCRMSpaces-elements including references
 * between them. If no such SCRMDiagram exists, this policy creates one and
 * links it to the space.
 * 
 * @author mharut
 * @generated NOT
 */
public class OpenSCRMSpaceEditPolicy extends OpenEditPolicy {
	...
	/**
	 * The command to open diagrams from an already existent
	 * <code>SCRMDiagram</code>. This command will open the diagram and
	 * initialize its content, if necessary.
	 * 
	 * @author mharut
	 * @generated NOT
	 */
	public static class OpenDiagramCommand extends AbstractTransactionalCommand {
		...
		/**
		 * Creates a new command to open the diagram linked by
		 * <code>linkStyle</code>.
		 * 
		 * @param linkStyle
		 *            the link style of the edit part to open the diagram for
		 */
		...
		protected CommandResult doExecuteWithResult(IProgressMonitor monitor,
			...
			try {
				...
				for (SCRMDiagram containingDiagram : representedSpace
				...
				scrmDiagram.getElements().addAll(
						getAllContents(representedSpace));
		...
		private SCRMDiagram intializeNewDiagram() throws ExecutionException {
		...
		private void addToParent(SCRMDiagram scrmDiagram) {
			...
			EStructuralFeature containingFeature = containee
					.eContainingFeature();
			List containedObjects = (List) container.eGet(containingFeature);
			containedObjects.add(scrmDiagram);
		}

		private Collection<SCRMModelElement> getAllContents(EObject parent) {
