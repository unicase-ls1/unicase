...
/**
 * This handler provides the possibility to open representing diagrams from
 * selecting the corresponding action from a SCRMSpace's context menu. To do so,
 * it uses a {@link OpenDiagramCommand}.
 * 
 * @author mharut
 */
public class OpenRepresentingDiagramHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selectedElement instanceof EditPart) {
			...
			new ICommandProxy(new OpenDiagramCommand(representedSpace))
					.execute();
	...
	/**
	 * Obtains the <code>EditPart</code> from the <code>event</code>'s current
	 * selection.
	 * 
	 * @param event
	 ...
	 *             if an exception occurred during the execution
	 */
	private Object obtainSelectedElement(ExecutionEvent event)
			throws ExecutionException {
