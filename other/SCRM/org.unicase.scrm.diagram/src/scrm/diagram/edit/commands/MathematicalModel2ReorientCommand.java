...
public class MathematicalModel2ReorientCommand extends EditElementCommand {
	...
	protected boolean canReorientSource() {
		...
		MathematicalModel target = (MathematicalModel) getLink().getSubMathematicalModels().get(0);
		if (!(getLink().eContainer() instanceof MathematicalModel)) {
			return false;
		}
		MathematicalModel container = (MathematicalModel) getLink().eContainer();
		return ScrmBaseItemSemanticEditPolicy.LinkConstraints.canExistMathematicalModel_4010(container, getNewSource(),
			target);
	}
	...
	protected boolean canReorientTarget() {
		...
		MathematicalModel container = (MathematicalModel) getLink().eContainer();
		return ScrmBaseItemSemanticEditPolicy.LinkConstraints.canExistMathematicalModel_4010(container, source,
			getNewTarget());
	}
