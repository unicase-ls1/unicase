 ...
 * @generated
 */
public class SCRMDiagramCanonicalEditPolicy extends CanonicalConnectionEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		for (Iterator it = ScrmDiagramUpdater.getSCRMDiagram_1000SemanticChildren(viewObject).iterator(); it.hasNext();) {
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize.add(ScrmPackage.eINSTANCE.getSCRMDiagram_NewElements());
		}
	...
	private Collection refreshConnections() {
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator.hasNext();) {
			...
			int diagramLinkVisualID = ScrmVisualIDRegistry.getVisualID(nextDiagramLink);
			...
			for (Iterator linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator.hasNext();) {
				ScrmLinkDescriptor nextLinkDescriptor = (ScrmLinkDescriptor) linkDescriptorsIterator.next();
	...
	private Collection collectAllLinks(View view, Map domain2NotationMap) {
		...
		case SCRMDiagramEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(ScrmDiagramUpdater.getSCRMDiagram_1000ContainedLinks(view));
			}
	...
	private Collection createConnections(Collection linkDescriptors, Map domain2NotationMap) {
		...
		for (Iterator linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator.hasNext();) {
			final ScrmLinkDescriptor nextLinkDescriptor = (ScrmLinkDescriptor) linkDescriptorsIterator.next();
