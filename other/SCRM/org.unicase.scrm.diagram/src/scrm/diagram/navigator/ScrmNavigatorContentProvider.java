...
public class ScrmNavigatorContentProvider implements ICommonContentProvider {
		...
		if (parentElement instanceof IFile) {
			...
			result
					.addAll(createNavigatorItems(selectViewsByType(resource
							.getContents(), SCRMDiagramEditPart.MODEL_ID),
							file, false));
	...
	private Object[] getViewChildren(View view, Object parentElement) {
		...
		case SCRMDiagramEditPart.VISUAL_ID: {
			...
			Collection connectedViews = getChildrenByType(Collections
					.singleton(view), ScrmVisualIDRegistry
		...
		case MathematicalModelEditPart.VISUAL_ID: {
			...
			incominglinks.addChildren(createNavigatorItems(connectedViews,
					incominglinks, true));
			connectedViews = getIncomingLinksByType(
					Collections.singleton(view), ScrmVisualIDRegistry
							.getType(MathematicalModel2EditPart.VISUAL_ID));
			incominglinks.addChildren(createNavigatorItems(connectedViews,
					incominglinks, true));
			connectedViews = getOutgoingLinksByType(
	...
	private Collection createNavigatorItems(Collection views, Object parent,
		...
		for (Iterator it = views.iterator(); it.hasNext();) {
			result
					.add(new ScrmNavigatorItem((View) it.next(), parent,
							isLeafs));
		}
		return result;
	}
