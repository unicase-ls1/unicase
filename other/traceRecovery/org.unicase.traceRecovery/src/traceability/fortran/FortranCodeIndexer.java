 ...
 *
 */
public class FortranCodeIndexer {
...
//		addComments(doc, parser);
	...
	private static void addSubroutines(Document doc,
		...
		ArrayList subroutines = (ArrayList<ScopingNode>) parser.getSubroutines();
		if (subroutines == null)
			return;
		for (int i = 0; i < subroutines.size(); i++) {
			String docSubroutine = ((ScopingNode) subroutines.get(i)).getName(true);
			doc.add(Field.Text(SUBROUTINE, docSubroutine));
		}
		
	}
	
	private static void addComments(Document doc,
			FortranSourceCodeParser parser) {
