/*
 * @(#)SwingSRView.java  1.3  2011-01-16
 *
 * Copyright (c) 2007-2011 Werner Randelshofer, Immensee, Switzerland.
 * All rights reserved.
 *
...
/**
 * IcerayView provides an interactive user interface for an {@link IcerayTree}.
 * <p>
 ...
 */
public class IcerayView extends JPanel implements SunrayViewer, TreeView {

    private IcerayDraw draw;
    private IcerayDraw subDraw;
    ...
    public IcerayView(SunrayTree model) {
        this.draw = new IcerayDraw(model.getRoot(), model.getInfo());
        this.model = model;
        init();
    }
    ...
    public void setDraw(IcerayDraw newValue) {
        this.draw = newValue;
    ...
    protected IcerayDraw createSubDraw(SunrayNode root, NodeInfo info) {
        if (draw instanceof MultilineIcerayDraw) {
        return new MultilineIcerayDraw(root,info);
        }else{
        return new IcerayDraw(root,info);
        }
    }
        ...
        private void showPopup(MouseEvent evt) {
            ...
            if (popupNode != null) {
                ...
                if (a.length > 0) {
                    ...
                    m.show(IcerayView.this, evt.getX(), evt.getY());
                }
            }
            evt.consume();
        }
