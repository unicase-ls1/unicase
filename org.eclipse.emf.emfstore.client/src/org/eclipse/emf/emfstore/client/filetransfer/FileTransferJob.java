...
/**
 * Abstract class for the file transfer job encapsulating methods used for
...
public abstract class FileTransferJob extends Job {
	 ...
	 *            the transfer manager handling the transfer job
	 * @param fileInfo
	 *            the transfer information of the job
	 * @param name
	 *            of the transfer job
	 */
	protected FileTransferJob(FileTransferManager transferManager,
			FileTransferInformation fileInfo, String name) {
		super(name);
	 ...
	 *             if there are any null values in the attributes
	 */
	...
	protected void setTotalWork(IProgressMonitor monitor) {
		monitor.beginTask("Transfering ", (int) (Math.ceil(fileInformation
				.getFileSize()) / FilePartitionerUtil.getChunkSize()));
	}
	...
	protected void initializeMonitor(IProgressMonitor monitor) {
		...
		// set progress based on how many file chunks that have already been
		// sent
	 ...
	 * 
	 * @param file
	 *            the file to store the transfer in
	 */
	...
	/**
	 * Returns the cache manager associated with the project space of this file
	 * transfer.
	 * 
	...
	/**
	 * Sets the exception for this transfer. Do this in case an exception
	 * occurs. This exception can later be retrieved by get exception.
	 * 
	 * @param exception
