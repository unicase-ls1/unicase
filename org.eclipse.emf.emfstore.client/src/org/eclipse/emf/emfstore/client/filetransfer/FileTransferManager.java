...
import org.eclipse.emf.emfstore.client.core.ProjectSpaceControllerImpl;
...
public class FileTransferManager {
	...
	private ProjectSpaceControllerInternal controller;
	...
	public FileTransferManager(ProjectSpace data,
			ProjectSpaceControllerInternal controller) {
		cacheManager = new FileTransferCacheManager(data);
		projectSpace = data;
	...
	private void addToCommitQueue(final FileIdentifier identifier) {
		for (FileIdentifier f : ((ProjectSpaceDataInternal) projectSpace)
				.getWaitingUploads()) {
		...
		new EMFStoreCommand() {
			...
			protected void doRun() {
				((ProjectSpaceDataInternal) projectSpace).getWaitingUploads()
						.add(identifier);
				// TODO: saveProjectSpaceOnly currently not in interface.
				// Check whether it is really needed there
				((ProjectSpaceControllerImpl) projectSpace)
	...
	public void uploadQueuedFiles(IProgressMonitor progress) {
		try {
			EList<FileIdentifier> uploads = ((ProjectSpaceDataInternal) projectSpace)
					.getWaitingUploads();
			while (!uploads.isEmpty()) {
				...
				if (!cacheManager.hasCachedFile(fi)) {
					...
					new EMFStoreCommand() {
						@Override
						protected void doRun() {
							((ProjectSpaceDataInternal) projectSpace)
									.getWaitingUploads().remove(fi);
							// TODO: saveProjectSpaceOnly currently not in
							// interface.
							// Check whether it is really needed there
							((ProjectSpaceControllerImpl) projectSpace)
									.saveProjectSpaceOnly();
	...
	private int getWaitingUploadIndex(FileIdentifier fileId) {
		 ...
		 * use it here.
		 */
		for (FileIdentifier upload : ((ProjectSpaceDataInternal) projectSpace)
				.getWaitingUploads()) {
	...
	void removeWaitingUpload(FileIdentifier fileId)
		...
		if (index != -1) {
			...
			// TODO: saveProjectSpaceOnly currently not in interface.
			// Check whether it is really needed there
			((ProjectSpaceControllerImpl) projectSpace).saveProjectSpaceOnly();
