...
/**
 * A Notification recorder is meant to generate a NotificationRecording. To
 * create the recording just repeatedly call record() with incoming EMF
 * notifications.
 * 
 * @author chodnick
 */
public class NotificationRecorder {
	...
	/**
	 * Returns a completed notification recording, may throw
	 * IllegalStateException in case the recording is requested while recording
	 * still takes place, i.e. the EMF notification chain is not completed yet.
	 * 
	...
	public NotificationRecording getRecording() {
		if (isRecordingComplete()) {
			...
			throw new IllegalStateException(
					"trying to get a recording, that is not finished yet");
		}
	}

	/**
	 * Tries to stop an ongoing recording, useful only in context of delete
	 * operations, in which multiple EMF notification chains are recorded as a
	 * single sequence.
	 */
	public void stopRecording() {
		...
		if (recording == null /* || recording.empty() */) {
			throw new IllegalStateException(
					"trying to close an empty recording");
		}
	 ...
	 * 
	 * @param aHint
	 *            the hint to use when creating the new recording.
	 */
	...
	/**
	 * @return true if current recording is completed and closed, false
	 *         otherwise
	 */
