...
public final class WorkspaceUtil {
	 ...
	 *            the message
	 * @param exception
	 *            the exception or null f not applicable
	 * @param statusInt
	 *            the status constant as defined in {@link IStatus}
	 */
	public static void log(String message, Exception exception, int statusInt) {
		Activator activator = Activator.getDefault();
		Status status = new Status(statusInt, activator.getBundle()
	 ...
	 *            the project space
	 * @param baseVersion
	 *            the base version that was checked out
	 */
	public static void logCheckout(ProjectSpaceInternal projectSpace,
			PrimaryVersionSpec baseVersion) {
		CheckoutEvent checkoutEvent = EventsFactory.eINSTANCE
				.createCheckoutEvent();
	 ...
	 *            the project space
	 * @param baseVersion
	 *            the base version of the project space
	 * @param targetVersion
	 *            the target version to update to
	 */
	public static void logUpdate(ProjectSpaceInternal projectSpace,
	 ...
	 *            the project space
	 * @param modelElement
	 *            the model element that is read
	 * @param sourceView
	 *            the view the read originates
	 * @param readView
	 *            the view the model element is shown in
	 */
	public static void logReadEvent(ProjectSpaceInternal projectSpace,
			ModelElementId modelElement, String sourceView, String readView) {
		...
		if (projectSpace == null) {
			logWarning(
					"Read event could not be logged since given project space was null",
	 ...
	 *            the project space
	 * @param sourceElement
	 *            the source element of the trace
	 * @param targetElement
	 *            the target event of the trace
	 * @param featureName
	 *            the feature reference which was traced.
	 */
	public static void logTraceEvent(ProjectSpaceInternal projectSpace,
			ModelElementId sourceElement, ModelElementId targetElement,
			String featureName) {
	...
	public static void logFocusEvent(String viewId) {
		...
		// TODO: we don't do anything we the project retrieved below
	...
	/**
	 * Opens a file with the default program used on the current computer. This
	 * is just the same as Desktop.open() of java 1.6. This should work with -
	 * Mac os X, - windows xp, - as well as the most linux distributions
	 * 
	 * @param fileUrl
	 *            the url of the file which shall be opened
	 */
	public static void openFile(String fileUrl) {
		...
		if (!cmd.equals("")) {
			try {
				...
				WorkspaceUtil.log(
						"could not open the file with the system dependant command: "
								+ cmd, e, IStatus.ERROR);
			}
		}
	}
	...
	public static EObject getEObjectByUri(URI uri) {
		...
		ResourceSet resourceSet = ((WorkspaceDataInternal) WorkspaceManager
				.getInstance().getCurrentWorkspace()).eResource()
				.getResourceSet();
