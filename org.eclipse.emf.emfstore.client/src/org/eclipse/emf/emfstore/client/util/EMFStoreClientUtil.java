...
public final class EMFStoreClientUtil {
	...
	/**
	 * Gives a server info for a given port and url. Searches first for already
	 * existing ones. If the search fails, it creates a new one and registers it
	 * for later lookup.
	 * 
	 * @param url
	 *            the server url (e.g. IP address or DNS name)
	 * @param port
	 *            the server port
	...
	private static ServerInfo giveServerInfo(String url, int port) {
		...
		for (ServerInfo existingServerInfo : workspace.getServerInfos()) {
			if (existingServerInfo.getName().equals(
					LOCALHOST_GENERATED_ENTRY_NAME)) {
				if (url.equals(existingServerInfo.getUrl())
						&& port == existingServerInfo.getPort()) {
	 ...
	 *            the certificateAlias (defaults to
	 *            {@link KeyStoreManager.DEFAULT_CERTIFICATE})
	 * @return a server info
	 */
	...
	/**
	 * Create a default user session with the default super user and password
	 * and a server on localhost on the default port.
	 * 
	...
	public static Usersession createUsersession(String username,
			String password, String serverUrl, int serverPort) {
		Workspace workspace = WorkspaceManager.getInstance()
				.getCurrentWorkspace();
		for (Usersession usersession : workspace.getUsersessions()) {
			...
			if (existingServerInfo != null
					&& existingServerInfo.getName().equals(
							LOCALHOST_GENERATED_ENTRY_NAME)
					&& existingServerInfo.getUrl().equals(serverUrl)
					&& existingServerInfo.getPort() == serverPort) {
				String encPassword = KeyStoreManager.getInstance().encrypt(
						password, existingServerInfo);
				if (username.equals(usersession.getUsername())
						&& encPassword.equals(usersession.getPassword())) {
					return usersession;
				}
			}
		}
	...
	/**
	 * Checks, if the given credentials can be authenticated at the given
	 * server.
	 * 
	 * @param username
	 *            the username
	 ...
	 *             Problem with the EMFStore Server
	 */
	public static boolean dryLogin(String username, String password,
			String serverUrl, int serverPort, String certificateAlias)
			throws EmfStoreException {
		Usersession usersession = ModelFactory.eINSTANCE.createUsersession();
