...
import org.eclipse.emf.emfstore.client.preferences.PreferenceManager;
...
public final class NotificationGenerator {
	...
	private static HashMap<ProjectSpaceInternal, NotificationGenerator> instances;

	private ProjectSpaceInternal projectSpace;
	 ...
	 *            the project space.
	 */
	private NotificationGenerator(ProjectSpaceInternal projectSpace) {
		...
		Collections.sort(allProviders, new Comparator<NotificationProvider>() {
			public int compare(NotificationProvider arg0,
				...
				return allProvidersMap.get(arg0).compareTo(
						allProvidersMap.get(arg1));
			}
		});
	}
	...
	public static NotificationGenerator getInstance(
			ProjectSpaceInternal projectSpace) {
		if (instances == null) {
			instances = new HashMap<ProjectSpaceInternal, NotificationGenerator>();
		}
		NotificationGenerator notificationGenerator = instances
				.get(projectSpace);
	...
	/**
	 * Generate notifications for a list of change packages. Will also generate
	 * an event for the generated notifications.
	 * 
	 ...
	 */
	public List<ESNotification> generateNotifications(
	 ...
	 * @param currentUsername
	 *            the name of the current user
	 * @param createGenerationEvent
	 *            true if an event should be generated about the notifications
	 * @return a list of notification
	 */
	public List<ESNotification> generateNotifications(
			List<ChangePackage> changePackages, String currentUsername,
		...
		for (List<ESNotification> esNotifications : generateNotificationsByProvider
				.values()) {
	...
	/**
	 * Generate notifications for a list of change packages, grouped by the
	...
	public Map<NotificationProvider, List<ESNotification>> generateNotificationsByProvider(
		...
		while (iterator.hasNext()) {
			...
			try {
				List<ESNotification> provideNotifications = current
		...
		if (createGenerationEvent) {
			NotificationGenerationEvent generationEvent = EventsFactory.eINSTANCE
					.createNotificationGenerationEvent();
