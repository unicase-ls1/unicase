...
public class WorkPackageImpl extends AnnotationImpl implements WorkPackage {
	 ...
	 * @see org.unicase.model.task.WorkPackage#getAllContainedWorkItems()
	 */
	public List<WorkItem> getAllContainedWorkItems() {
		List<WorkItem> ret = new ArrayList<WorkItem>();
		EList<WorkItem> containedWorkItems = getContainedWorkItems();
		ret.addAll(containedWorkItems);
		for (WorkItem workItem : containedWorkItems) {
			if (workItem instanceof WorkPackage) {
				ret.addAll(((WorkPackage) workItem).getAllContainedWorkItems());
			}
		}
		return ret;
	}
