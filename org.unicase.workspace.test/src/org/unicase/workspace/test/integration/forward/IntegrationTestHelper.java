...
public final class IntegrationTestHelper {
	...
	private List<EObject> allMEsInProject;
	...
	public List<EObject> getRandomMEs(Project project, int num, boolean unique) {
		...
		if (unique) {
			do {
				final EObject me = allMEsInProject.get(getRandom().nextInt(numOfMEs - 1));
	...
	public EObject createRandomME() {
		Set<EClass> eClazzSet = ModelUtil.getSubclasses(EcoreFactory.eINSTANCE.createEObject().eClass());
		ArrayList<EClass> eClazz = new ArrayList<EClass>(eClazzSet);
		EClass eClass = eClazz.get(getRandom().nextInt(eClazz.size() - 1));
		EObject me = (EObject) eClass.getEPackage().getEFactoryInstance().create(eClass);
	...
	public void doContainmentRefTransitiveChange() {
		...
		while (refToChange == null || meB == null || meC == null) {
			while (meToMove == null) {
				...
				if (contentsSize != 0) {
					meToMove = (EObject) meA.eContents().get(getRandomPosition(contentsSize));
				}
	...
	public void doDeleteAndRevert() {
		...
		ModelUtil.getProject(modelElement).deleteModelElement(modelElement);
	...
	public void doNonContainmentReferenceRemove() {
		...
		EObject meToRemove = (EObject) me.eCrossReferences().get(indexToRemove);
