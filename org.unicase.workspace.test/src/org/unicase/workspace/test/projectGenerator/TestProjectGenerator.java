...
public class TestProjectGenerator {
	...
	private static final EClass MODELELEMENT_ECLASS = EcoreFactory.eINSTANCE.createEObject().eClass(); // MetamodelPackage.eINSTANCE.getModelElement();
	...
	private void createNormalReference(EObject me, EReference ref, int lowerBound, int upperBound) {
		...
		// getNumOfRefs())
		/*
		 * if (instancesOfRefType.size() < numOfRefs) { int lackingInstances = numOfRefs - instancesOfRefType.size();
		 * boolean noSection = true; for (int i = 0; i < lackingInstances; i++) { // 1.we don't want to create
		 * Sections!! // 2.createInstance() adds also the newly created instances to // tracking hash-tables EObject
		 * newInstance = createInstance(ref.getEmReferenceType(), noSection); instancesOfRefType.add(newInstance); // if
		 * it's a ModelElement, add it to project too // TODO // if (newInstance instanceof ModelElement) {
		 * project.addModelElement(newInstance); // } } } List<EObject> referencedInstances = new ArrayList<EObject>();
		 * for (int i = 0; i < numOfRefs; i++) { // pick random instance of ref's type int index =
		 * random.nextInt(instancesOfRefType.size()); EObject referencedInstance = instancesOfRefType.get(index);
		 * referencedInstances.add(referencedInstance); // remove the referenced instance, so that it's // not
		 * incidentally referenced again. instancesOfRefType.remove(referencedInstance); } if (ref.isMany()) {
		 * List<Object> list = (List<Object>) me.eGet(ref); list.addAll(referencedInstances); } else if
		 * (referencedInstances.size() != 0) { me.eSet(ref, referencedInstances.get(0)); } else { // me.eSet(ref, null);
		 * }
		 */
	}
	...
	private void distributeMEsOnLeafSection(LeafSection ls) {
		...
		if (freeMEs.size() < numOfRefs) {
			...
			for (int i = 0; i < lackingFreeInstances; i++) {
				...
				// create Sections
				freeMEs.add(createInstance(EcoreFactory.eINSTANCE.createEObject().eClass(), true));
