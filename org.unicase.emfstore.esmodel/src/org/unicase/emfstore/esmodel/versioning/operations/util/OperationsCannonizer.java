...
public final class OperationsCannonizer {
	...
	private static void handleMultiRefOperation(
			MultiReferenceOperation multiReferenceOperation,
			Map<String, ReferenceOperation> changedReferences,
	...
	private static void aggregateMultiwithMultiReference(
			MultiReferenceOperation multiReferenceOperation,
	...
	private static void aggregateMultiwithSingleReference(
			MultiReferenceOperation multiReferenceOperation,
			Map<String, ReferenceOperation> changedReferences,
			Set<AbstractOperation> operationsToBeDeleted, String key,
	...
	private static void handleSingleRefOperation(
	...
	private static void handleAttributeOperation(
		...
		if (changedAttributes.containsKey(key)) {
			...
			if (lastAttributeOperation.getNewValue()!=null && lastAttributeOperation.getNewValue().equals(
					lastAttributeOperation.getOldValue())) {
				operationsToBeDeleted.add(attributeOperation);
				changedAttributes.remove(key);
			}
		} else {
			if (attributeOperation.getNewValue()!=null && attributeOperation.getNewValue().equals(
	...
	private static void handleCreateDeleteOperation(
			Map<ModelElementId, AbstractOperation> deletedElements,
			HashMap<ModelElementId, Set<AttributeOperation>> modelElementAttributeChangeMap,
			Set<AbstractOperation> operationsToBeDeleted,
