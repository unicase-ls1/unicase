...
public class ExportDialog extends TitleAreaDialog {
	...
	private static final String DEFAULT_EXPORT_PATH = System
	 ...
	 * 
	 * @param parentShell
	 *            the parent shell object
	 * @param docRenderer
	 *            the DocumentRenderer which will use the template to render it
	 *            to an URootCompositeSection, which is used by the DocWriter
	 * @param modelElement
	 *            the modelElement which shall be exported to a document
	 */
	public ExportDialog(Shell parentShell, DocumentRenderer docRenderer,
	...
	protected Control createDialogArea(Composite parent) {
		...
		try {
			...
			MessageBox finished = new MessageBox(ExportDialog.platformShell,
					SWT.OK | SWT.ERROR);
		...
		fileLocation.setText(PreferenceHelper.getPreference(
				EXPORT_PATH_QUALIFIER, DEFAULT_EXPORT_PATH));
		...
		selectFileLocation
				.addSelectionListener(new FileLocationSelectionListener());
		...
		for (int i = 0; i < docWriters.size(); i++) {
			...
			exportType.add(writer.getFileType() + " ("
					+ writer.getClass().getSimpleName() + ")", i);
		}
		exportType.select(0);
		...
		recursionDepth.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				DocumentExport.setRecursionDepth(recursionDepth
						.getSelectionIndex() + 1);
			}
		});
			...
			modelElementAsLeafSection
					.addSelectionListener(new SelectionListener() {
						...
						public void widgetSelected(SelectionEvent e) {
							DocumentExport
									.setTreatModelElementAsLeafSection(((Button) e.widget)
											.getSelection());
						}
					});
		...
		saveAndOpenButton
				.addSelectionListener(new SaveAndOpenSelectionListener());
	...
	private void exportDocument(DocumentExport docExport, String fileUrl) {
		...
		try {
			...
			finished.setMessage("Fatal Error: "
	...
	/**
	 * Check if the file already exists and returns true, if the file shall be
	 * written. If the file already exists, a prompt message box will appear,
	 * which asks the user, if he really wants to overwrite the file.
	 * 
	 * @param fileUrl
	 *            the system dependent url to the file
	...
	private boolean checkFileName(String fileUrl) {
		...
		try {
			...
			String err = "The file name '" + fileUrl
					+ "' you entered is invalid.\n";
			MessageDialog.openError(Display.getCurrent().getActiveShell(),
					"Invalid file name", err);
			return false;
		}
		...
		if (f.exists()) {
			...
			try {
				...
				MessageBox messageBox = new MessageBox(
						ExportDialog.platformShell, SWT.YES | SWT.NO
								| SWT.ICON_WARNING | SWT.CENTER);
				messageBox.setMessage("The file '" + fileUrl
				...
				MessageBox messageBox = new MessageBox(
						ExportDialog.platformShell, SWT.OK | SWT.ICON_ERROR
								| SWT.CENTER);
	...
	private DocumentExport createDocumentExport() {
		...
		DocumentExport docExport = new DocumentExport(modelElement, docWriter,
				templates.get(template.getSelectionIndex()), docRenderer);
	...
	private String getFileUrl() {
		String fileUrl = fileLocation.getText() + File.separatorChar
				+ fileName.getText() + "."
	...
	/**
	 * Listener for the save button, which simply exports the document to a
	 * file.
	 */
		...
		public void widgetSelected(SelectionEvent e) {
			if (fileName.getText().length() < 1) {
				...
				if (checkFileName(fileUrl)) {
					PreferenceHelper.setPreference(EXPORT_PATH_QUALIFIER,
	...
	/**
	 * Listener for the save and open button, which exports the document and
			...
			if (fileName.getText().length() < 1) {
				...
				if (checkFileName(fileUrl)) {
					...
					try {
						...
						ModelUtil.logException("Opening file " + fileUrl
								+ " failed!", fte);
					}
					close();
				}
	...
	class FileLocationSelectionListener extends SelectionAdapter {
		...
		@Override
		public void widgetSelected(SelectionEvent e) {
			DirectoryDialog fd = new DirectoryDialog(((Button) e.widget)
					.getParent().getShell());
