...
public class OperationsPackageImpl extends EPackageImpl implements OperationsPackage {
	...
	public void initializePackageContents() {
		...
		initEClass(pushDownAttributeOperationEClass, PushDownAttributeOperation.class, "PushDownAttributeOperation",
			!IS_ABSTRACT, !IS_INTERFACE, IS_GENERATED_INSTANCE_CLASS);
		initEReference(getPushDownAttributeOperation_Attribute(), theMetamodelPackage.getModelElementId(), null,
			"attribute", null, 1, 1, PushDownAttributeOperation.class, !IS_TRANSIENT, !IS_VOLATILE, IS_CHANGEABLE,
			IS_COMPOSITE, !IS_RESOLVE_PROXIES, !IS_UNSETTABLE, IS_UNIQUE, !IS_DERIVED, IS_ORDERED);

		op = addEOperation(pushDownAttributeOperationEClass, theClassesPackage.getAttribute(), "getAttribute", 1, 1,
			IS_UNIQUE, IS_ORDERED);
		addEParameter(op, theMetamodelPackage.getProject(), "project", 0, 1, IS_UNIQUE, IS_ORDERED);
	...
	protected void createEcoreAnnotations() {
		...
		addAnnotation(extractSuperClassOperationEClass.getEOperations().get(4), source, new String[] { "documentation",
			"A class or enumeration with that name already exists." });
		addAnnotation(inlineClassOperationEClass.getEOperations().get(1), source, new String[] { "documentation",
			"The association must be a composition." });
		addAnnotation(inlineClassOperationEClass.getEOperations().get(2), source, new String[] { "documentation",
			"The multiplicity of the association must be 1-to-1." });
		addAnnotation(inlineClassOperationEClass.getEOperations().get(3), source, new String[] { "documentation",
			"The class to be inlined must not have sub classes." });
		addAnnotation(inlineClassOperationEClass.getEOperations().get(4), source, new String[] { "documentation",
