...
/**
 * A combo view is a wrapper around a graphical SWT Combo element.
 * It provides type-safe retrieval of the selected element and allows
 * to easily track selection changes. While a combo only allows String 
 * values, this view allows arbitrary values in the model.
 * 
 * The combo view is a wrapper for Combo, comparable to JFace wrappers
 * (like TreeView wrapping Tree).
 * It supports LabelProviders but no ContentProviders.
 * Only the text, not the image, is taken from the label provider,
 * since the Combo object does not support images.
 * 
 * @author jfinis
 *
...
public class ComboView<T> {
	...
	/**
	 * Sets the input of this combo box.
	 * Will have the first element selected by default.
	 * and will not fire a selection changed event.
	 * @param entries
	 */
	...
	/**
	 * Sets the input for this combo box
	 * You can set which of the entries is selected by choosing one as default entry.
	 * 
