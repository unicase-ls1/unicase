		...
		for (int d = 0; d < max_d; d++) {
			...
			for (int k = -d; k <= d; k += 2) {
				...
				if (x == text1_length && y == text2_length) {
					...
					LinkedList<Diff> a = diff_path1(v_map1, text1.substring(0,
							x), text2.substring(0, y));
					a.addAll(diff_path2(v_map2, text1.substring(x), text2
							.substring(y)));
					return a;
				}
			}
			...
			if (doubleEnd) {
				...
				for (int k = -d; k <= d; k += 2) {
					...
					if (done) {
						...
						LinkedList<Diff> a = diff_path1(v_map1, text1
								.substring(0, text1_length - x), text2
								.substring(0, text2_length - y));
						a.addAll(diff_path2(v_map2, text1
								.substring(text1_length - x), text2
				...
				if (v_map.get(d).contains(diff_footprint(x - 1, y))) {
					...
					if (last_op == Operation.DELETE) {
						...
						path.addLast(new Diff(Operation.DELETE, text1
								.substring(text1.length() - x - 1, text1
										.length()
										- x)));
					}
					last_op = Operation.DELETE;
					...
					if (last_op == Operation.INSERT) {
						...
						path.addLast(new Diff(Operation.INSERT, text2
								.substring(text2.length() - y - 1, text2
										.length()
										- y)));
					}
					last_op = Operation.INSERT;
	...
	public String diff_prettyHtml(LinkedList<Diff> diffs) {
		...
		for (Diff aDiff : diffs) {
			...
			case DELETE:
				...
				break;
			case EQUAL:
				html.append("<SPAN TITLE=\"i=").append(i).append("\">").append(
						text).append("</SPAN>");
				break;
			}
			if (aDiff.operation != Operation.DELETE) {
	...
	public Object[] patch_apply(LinkedList<Patch> patches, String text) {
		...
		for (Patch aPatch : patches) {
			...
			if (text1.length() > this.Match_MaxBits) {
				...
				// case of
				// a monster delete.
				start_loc = match_main(text, text1.substring(0,
