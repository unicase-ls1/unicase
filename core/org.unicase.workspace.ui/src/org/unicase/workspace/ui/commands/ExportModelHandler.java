...
public class ExportModelHandler extends AbstractHandler {
	...
	public static final String[] FILTER_NAMES = {
			"Unicase Project Files (*.ucm)", "All Files (*.*)" };
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (exportModelElements.size() > 0) {
			...
			String filePath = getFilePathByFileDialog(UiUtil
					.getNameForModelElement(exportModelElements.get(0)));
	...
	private void runCommand(final List<ModelElement> exportModelElements,
			String filePath) {
			...
			protected void doRun() {
				try {
					...
					progressDialog.getProgressMonitor().beginTask(
							"Export modelelement...", 100);
					progressDialog.getProgressMonitor().worked(10);
					ModelUtil2.saveObjectToResource(exportModelElements, uri);
					MessageDialog.openInformation(null, "Export",
							"Exported modelelement to file " + file.getName());
				} catch (IOException e) {
	...
	private List<ModelElement> getSelfContainedModelElementTree(
		...
		if (selection != null && selection instanceof IStructuredSelection) {
			...
			if (firstElement instanceof ModelElement) {
				...
				// only export the rootnode makes xml with references, otherwise
				// (see (commented) line two) the children
				// will be "real" nested as containments of the node (is not
				// necessary)
	...
	private String getFilePathByFileDialog(String modelElementName) {
		...
		try {
			...
			String initialFileName = "ModelElement_" + modelElementName
					+ ".ucm";
