...
public abstract class AbstractParser implements IParser {
	...
	protected IParserEditStatus validateNewValues(Object[] values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (value instanceof InvalidValue) {
				return new ParserEditStatus(org.unicase.ui.diagram.stateDiagram.part.ModelDiagramEditorPlugin.ID,
			...
			if (Boolean.TYPE.equals(iClass)) {
				if (value instanceof Boolean) {
					...
					value = new InvalidValue(NLS.bind(
						org.unicase.ui.diagram.stateDiagram.part.Messages.AbstractParser_UnexpectedValueTypeMessage,
						iClass.getName()));
				}
					...
					if (s.length() == 0) {
						...
						try {
							...
							value = new InvalidValue(
								NLS
									.bind(
										org.unicase.ui.diagram.stateDiagram.part.Messages.AbstractParser_WrongStringConversionMessage,
										iClass.getName()));
						}
					}
				} else {
				...
				if (value instanceof String) {
					...
					if (literal == null) {
						value = new InvalidValue(NLS.bind(
							org.unicase.ui.diagram.stateDiagram.part.Messages.AbstractParser_UnknownLiteralMessage,
							value));
					} else {
