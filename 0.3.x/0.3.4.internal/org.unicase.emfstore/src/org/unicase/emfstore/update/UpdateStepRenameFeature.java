...
/**
 * @author schroech "Renames" a feature by transferring the value of one feature to another
 */
public abstract class UpdateStepRenameFeature extends UpdateStepTransformClass {
	...
	public int updateModelElement(ModelElement modelElement) {
		...
		if (sourceValue == null || ((sourceValue instanceof EList) && ((EList) sourceValue).size() == 0)) {
			return 0;
		}
		...
		System.out.println("Renamed feature \"" + getSourceFeatureName() + "\" of model element \""
			+ modelElement.getName() + "\" to \"" + getTargetFeatureName() + "\"");
