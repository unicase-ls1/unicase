var TinyMCE_EclipseTablePlugin = {
	/**
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		if (tdElm = tinyMCE.getParentElement(node, "td,th")) {
			window.status="34:" + start;
			window.status="status:normal";
			window.status="35:" + start;
			window.status="status:normal";
			window.status="36:" + start;
			window.status="status:normal";
			window.status="37:" + start;
			window.status="status:normal";
			window.status="38:" + start;
			window.status="status:normal";
			window.status="39:" + start;
			window.status="status:normal";
		...
		window.status="32:" + start;
		var tableElement;
		// Within table
		if (tinyMCE.getParentElement(node, "table")) {
			tableElement = tinyMCE.getParentElement(node, "table");
			window.status="status:selected";
			window.status="cellpadding:" + tableElement.getAttribute('cellpadding');
			window.status="cellspacing:" + tableElement.getAttribute('cellspacing');
			window.status="align:" + tableElement.getAttribute("align");
			window.status="class:" + tinyMCE.getVisualAidClass(tableElement.getAttribute("class"), false);
			window.status="style:" + tinyMCE.serializeStyle(tableElement.getAttribute("style"));
			window.status="id:" + tableElement.getAttribute("id");
...
tinyMCE.addPlugin("eclipsetable", TinyMCE_EclipseTablePlugin);