...
/**
 * @author jfinis This static class contains utility methods for creating
 *         multi-asignee-actions.
 */
public final class MultiActionGenerator {
	...
	private static void copyValues(ActionItem from, WorkItem to) {
		...
		to.getAppliedStereotypeInstances().addAll(
				from.getAppliedStereotypeInstances());
		to.getAttachments().addAll(from.getAttachments());
		to.getComments().addAll(from.getComments());
		to.getIncomingDocumentReferences().addAll(
				from.getIncomingDocumentReferences());
	...
	/**
	 * private recursive method used by flattenOrgList. Not to be used anywhere
	 * else.
	 * 
	 * @param out
	 *            The resulting user set
	 * @param visitedGroups
	 *            A set containing visited groups
	 * @param toAdd
	 *            A list to be flattened recursively
	 */
	private static void flattenRecursive(Set<User> out,
			Set<Group> visitedGroups, List<OrgUnit> toAdd) {
		for (OrgUnit o : toAdd) {
			if (o instanceof User) {
				out.add((User) o);
			} else if (o instanceof Group) {
				// Track visited groups for
				// circular-group-looping-recursion-avoidance
				if (!visitedGroups.contains(o)) {
					visitedGroups.add((Group) o);
					flattenRecursive(out, visitedGroups,
							((Group) o).getOrgUnits());
				}
			}
		}
	...
	/**
	 * Flattens an organization list, i.e. a list of user and/or groups. The
	 * flattened list is returned as a user list. Flattening means all groups
	 * are "unpacked" recursively and all users in them are added to the result
	 * list. The method keeps track of visited groups, so cycles between groups
	 * will not create an endlessly looping recursion (resulting in a stack
	 * overflow).
	 * 
	 * @param in
	 *            a List of OrgUnits to be flattened
	 ...
	 * It splits an action item into many copies (one for each User in the
	 * assignee input list). The copies are exact copies of the old action item,
	 * only the assignee is set to the one taken from the list. In addition, a
	 * WorkPackage is created at the location of the input actionItem. This
	 * WorkPackage also gets copied most values from the input actionItem. All
	 * copies of actionItem are inserted into the newly created work package.
	 * Finally, the input actionItem is deleted. All this is done in a composite
	 * operation, so the history isn't spammed with a huge amount of actions.
	 * 
	 * @param baseAction
	 *            An action item to be assigned to many users, will be split and
	 *            deleted.
	 * @param assignees
	 *            A list of assignees for this task
	 * @return The resulting WorkPackage containing all newly created
	 *         ActionItems.
	 */
	public static WorkPackage generateMultiAction(ActionItem baseAction,
			List<User> assignees) {
		// Leaf Section
		// Project
		// Work Package
		...
		try {
			projectSpace = currentWorkspace.getProjectSpace(project);
		...
		// Set values for the resulting work package to those of the old Work
		// Item
		...
		for (User curAssignee : assignees) {
			...
			// Copy values (not good if ActionItem gets new attributes, a .clone
		...
		try {
			operationHandle.end("Assign action item to group",
					"Assigned action item " + baseAction.getName()
							+ " to multiple assignees.",
					project.getModelElementId(result));
