 ...
 * $Id$
 */
package org.unicase.ui.meeditor.mecontrols.issuecontrol;
...
public class AssessmentMatrixControl extends AbstractMEControl{
	...
	private void rebuildMatrix() {
		...
		for(int i = 0; i < criteria.size(); i++){
			final Hyperlink hyperlink = getToolkit().createHyperlink(matrixSection, cutStringName(criteria.get(i).getName()), parentStyle);
		...
		for(int p= 0; p < proposals.size(); p++) {			
			...
			final Hyperlink hyperlink = getToolkit().createHyperlink(matrixSection, currentProposal.getName(), parentStyle);
			...
			allSumLabels.put(currentProposal,getToolkit().createLabel(sumLabelComposite, " "+ computeAssessmentSum(currentProposal)+" "));
	...
	private Assessment getAssessment(final Proposal p, final Criterion c){
		...
		for(int i = 0; i<assessmentsOfProposal.size(); i++) {
			if(assessmentsOfProposal.get(i).getCriterion() != null && assessmentsOfProposal.get(i).getCriterion().equals(c)) {
				return assessmentsOfProposal.get(i);
			}
		}
		return null;
