...
public class UCDropAdapter extends DropTargetAdapter {
		...
		domain.getCommandStack().execute(
				new RecordingCommand((TransactionalEditingDomain) domain) {
					...
					protected void doExecute() {
						...
						EReference theRef = getTargetRef(target, source);
						if (theRef == null) {
							return;
						}
	...
	private EReference getTargetRef(ModelElement target,
			List<ModelElement> source) {
	...
	protected void helper(DropTargetEvent event) {
		...
		event.detail = DND.DROP_COPY;// | DND.DROP_MOVE | DND.DROP_LINK;
...
//
// /**
// * This implementation of a drop target listener is designed to turn a drag
// and drop
// * operation into a {@link Command} based on the model objects of an
// * {@link EditingDomain} and created by {@link DragAndDropCommand#create}. It
// is
// * designed to do early data transfer so the the enablement and feedback of
// the
// * drag and drop interaction can intimately depend on the state of the model
// objects
// * involved. On some platforms, however, early data transfer is not available,
// so this
...
// * which short-circuits the transfer process for simple transfers within the
// workbench,
// * the method {@link #getDragSource} can be overridden to change the
// behaviour.
// * The implementation also only handles an {@link IStructuredSelection},
// * but the method {@link #extractDragSource} can be overridden to change the
// behaviour.
// * <p>
// * SWT's {@link DND#FEEDBACK_SCROLL auto-scroll} and {@link
// DND#FEEDBACK_EXPAND auto-expand}
// * (hover) are enabled by default. The method {@link #getAutoFeedback} can be
// overridden
...
//
// /**
// * This is the command created during dragging which provides the feedback and
// will carry out the action upon completion.
// */
...
// command = DragAndDropCommand.create(domain, target, getLocation(event),
...
// // }
// //HACK: I extract drag source from viewer. It is set in
// UCDragAdapter.dragStart() event.
// source = (List<ModelElement>) viewer.getData(DRAG_SOURCE_KEY);
//		
// if (source == null) {
// return;
...
// dragAndDropCommandInformation = new DragAndDropCommandInformation(domain,
// target, location, event.operations, originalOperation, source);
//
...
// //
// valid = ((DragAndDropFeedback)command).validate(target, location,
// event.operations, originalOperation, source);
//
...
// retrieved from the transfer agent.
// * This default implementation converts a structured selection into a
// collection of elements.
// */
// // protected Collection<?> extractDragSource(Object object)
// // {
// // // Transfer the data and convert the structured selection to a collection
// of objects.
// // //
...
// return
// item.getData(org.eclipse.emf.common.ui.viewer.ExtendedTableTreeViewer.ITEM_ID
// ) instanceof Item ?
// ((Item)item.getData(org.eclipse.emf.common.ui.viewer.ExtendedTableTreeViewer.
// ITEM_ID)).getData() :
// item.getData();
// }
//
...
// (EditingDomain domain, Object target, float location, int operations, int
// operation, Collection<?> source)
// {
// this.domain = domain;
// this.target = target;
...
// {
// return DragAndDropCommand.create(domain, target, location, operations,
// operation, source);
// }
// }
// }
